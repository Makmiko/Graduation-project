...
namespace Makmiko\Project\Core;
...
class DBConnection
{
    ...
    public function queryAll($sql){
        $statement = $this->connection->query($sql); // Для статических запросов
        if (!$statement) return false;
        return $statement->fetchAll();
    }

    public function exec($sql){
        return $statement = $this->connection->exec($sql); // Для запросов без ответа
    }

    public function execute($sql, $params, $all = true){ // Для подготовленных запросов
    ...
    public function executeSql($sql, $params){
        $statement = $this->connection->prepare($sql); // Подготовленные запросы без ответов
        $statement->execute($params);
    }
}